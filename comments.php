...
<?php if($comments) : //如果有评论 ?>
  ...
  <?php if ( comments_open() ) : /**/ ?>
    ...
    <h3 class="comment-counts"><a href="#respond_box">发表评论</a></h3>
...
<?php if(comments_open()) : ?>
  ...
  <div id="respond">
    ...
    <?php if ( get_option('comment_registration') && !$user_ID ) : ?>
      ...
      <div class="comment-tip"><p>You must be <a href="<?php echo wp_login_url(); ?>?redirect_to=<?php echo urlencode(get_permalink()); ?>">登陆</a> 以评论</p></div>
    ...
    <?php else : ?>
      ...
      <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
        ...
        <?php if ( $user_ID ) : ?>
          ...
          <div class="comment-tip"><p>
           <?php {
              ...
              $link = 'Logged in as: '.$user->display_name.'  <a href="' . get_option('siteurl') . '/wp-login.php?action=logout">Logout &raquo;</a>';
        ...
        <p>
          <input type="email" name="email" id="email" class="commenttext" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" placeholder="邮箱 (必填,非公开)" required />
        </p>
        <p>
          <input type="text" name="url" id="url" class="commenttext" value="<?php echo $comment_author_url; ?>" size="22" tabindex="3" placeholder="链接" />
        </p>
          </div>
        ...
        <div class="comment-submit"><input class="submit" name="submit" type="submit" id="submit" tabindex="5" value=" 提交 " /></div>
        <div class="clear"></div>
